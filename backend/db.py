...
def init_db(conn):
    with conn.cursor() as cur:
        ...
        cur.execute(
            "UPSERT INTO activities (activity, category) VALUES ('Flying', 'Absolute Darkness')")
...
def return_activity(conn):
    with conn.cursor() as cur:
        cur.execute("SELECT * FROM activities ORDER BY RANDOM() LIMIT 1")
        logging.debug("delete_activity(): status message: %s",
                      cur.statusmessage)
        result = cur.fetchall()
        conn.commit()
...
def print_activities(conn):
    with conn.cursor() as cur:
        cur.execute("SELECT activity, location, transportation FROM activities")
...
def main():
    ...
    delete_activity(conn, "Hiking")
    print_activities(conn)

    insert_activity(
        conn, {
            'activity': 'Helicoptering', 'category': 'Sandstorm'})
    print_activities(conn)

    return_activity(conn)
